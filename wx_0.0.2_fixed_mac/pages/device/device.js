...
Page({
  data: {
    ...
    SendTimer: null,
    fixedDeviceID: 'CD:2B:87:FB:FD:87',
    msgSentFlag: true
  },
  ...
  bindInputID: function (e) {
    var that = this
    that.data.fixedDeviceID = e.detail.value;
    //this.setData({
    //  inputText: e.detail.value
    //})
    console.log(e.detail.value)
  ...
  autoConnect: function () {
    ...
    for (var i = 0; i < that.data.devicesList.length; i++) {
      if (that.data.fixedDeviceID == that.data.devicesList[i].deviceId) {
    ...
    wx.createBLEConnection({
      deviceId: that.data.fixedDeviceID,
      success: function (res) {
        console.log(res)
        /*
        wx.hideLoading()
        ...
        that.setData({
          connected: true,
          connectedDeviceId: that.data.fixedDeviceID,
          name: name
        })
        wx.nextTick(() => {
          that.ConnectReady()
  ...
  onLoad: function (options) {
    ...
    wx.onBluetoothDeviceFound(function (devices) {
      ...
      if(!that.data.msgSentFlag){
        for (var i = 0; i < that.data.devicesList.length; i++) {
          if (that.data.devicesList[i].deviceId == that.data.fixedDeviceID) {
            that.setData({
              msgSentFlag: true
            })
